...
当前仓库已有初始提交。后端已具备健康检查、数据库迁移、账号注册登录、JWT 鉴权、当前用户接口、轨迹列表、轨迹详情、轨迹编辑、轨迹删除、用户数据隔离、版本冲突检测、航班导入和火车导入。Web 前端已具备 API 客户端、健康检查状态、登录注册、token 持久化、轨迹列表拉取、Google Maps 地图展示、轨迹点和轨迹线绘制、选中高亮、航班导入、火车导入和轨迹详情编辑面板。Android App 已具备 Compose 工程骨架、登录注册、token 持久化、网络层、轨迹列表页面、Google 地图展示页面、航班导入页和火车导入页。
