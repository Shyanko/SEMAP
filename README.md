...
- APK 下载入口指向当前正式版本 `SEMAP-1.2.apk`。
- 后端健康状态显示。
...
```bash
https://semap.xyz/downloads/SEMAP-1.2.apk
```

当前 APK 正式版本为 `1.2`，发布文件命名格式为 `SEMAP-版本号.apk`。当前包使用 `versionCode=102`、`versionName=1.2`、`minSdk=26`、`targetSdk=35`，编译环境为 Android SDK 36。Nginx 对 `.apk` 下载返回 `application/vnd.android.package-archive`。

发布 APK 校验：

```bash
AAPT=$(find "$ANDROID_HOME" -name aapt -type f | sort | tail -n 1)
"$AAPT" dump badging /var/www/semap/downloads/SEMAP-1.2.apk

APKSIGNER=$(find "$ANDROID_HOME" -name apksigner -type f | sort | tail -n 1)
"$APKSIGNER" verify --verbose --print-certs /var/www/semap/downloads/SEMAP-1.2.apk

curl -I -L https://semap.xyz/downloads/SEMAP-1.2.apk
```
