...
- APK 下载入口指向当前正式版本 `SEMAP-1.4.apk`。
- 后端健康状态显示。
...
- 地图页切换 Google 或高德后，切到其他页再返回会保持当前地图源。
...
- 轨迹列表中选中卡片后显示“更改”按钮，弹窗内可修改标题、起止时间、保存或删除路径。
- 航班导入页标题为“航班号导入”，火车导入页标题为“车次号导入”。
...
```bash
https://semap.xyz/downloads/SEMAP-1.4.apk
```

当前 APK 正式版本为 `1.4`，发布文件命名格式为 `SEMAP-版本号.apk`。当前包使用 `versionCode=104`、`versionName=1.4`、`minSdk=26`、`targetSdk=35`，编译环境为 Android SDK 36。Nginx 对 `.apk` 下载返回 `application/vnd.android.package-archive`。
