...
10. 恢复网络后补传定位点。
11. 查看轨迹详情。
12. 在轨迹列表选中卡片后修改标题、起止时间或删除路径。
13. 轨迹列表按出发时间升序排列。
14. 地图页不显示轨迹详情卡片，地图容器占满内容区剩余空间。
15. 地图页切换 Google 或高德后，切到其他页再返回保持当前地图源。
16. 导入页和 GPS 记录页在小屏幕上必须可纵向滚动。
...
地图展示能力使用 Google Maps 和高德地图：

- Web 前端使用 Google Maps JavaScript API。
- Web 前端支持切换高德 JSAPI 底图。
- Android App 使用 Google Maps SDK for Android。
- Android App 支持切换高德地图 Android SDK 底图。
...
8. 航司 logo 由后端通过 `/api/assets/airline-logos/{code}.png` 获取并缓存，客户端不直接访问第三方 logo 源。
9. 航空公司查询失败时返回导入错误，不写入运营方兜底值。
10. 起飞地点和降落地点使用 IATA code-search 将 IATA 代码解析为城市和机场名。
11. 返回生成的 `TrackSegment`。
