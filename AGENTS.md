...
- 坐标生成：后端使用 Google Geocoding 将站名转换成坐标。
...
1. `POST /api/segments` 创建轨迹，不携带 `version`，返回 `version=1`。
2. `PATCH /api/segments/{segmentId}` 必须在请求体携带当前 `version`。
3. `DELETE /api/segments/{segmentId}` 必须在 query 参数携带当前 `version`。
4. 版本不一致时返回 `409`。
5. 普通轨迹点不提供独立批量写入接口。手动轨迹和外部导入轨迹由创建或导入接口一次性写入点位。
...
2. 先通过 `https://search.12306.cn/search/v1/train/search` 获取内部 `train_no`。
3. 精确匹配 `station_train_code` 等于用户输入车次号的结果。
4. 再通过 `queryTrainInfo/query` 获取车次经停站。
5. 将站点转换成坐标。
6. 按站点顺序生成轨迹点。
7. 设置 `isApproximate=true`。
8. 摘要中写明按服务器当前日期车次近似生成。
...
2. 会话点位只允许通过定位会话接口上传。
3. 暂停时会话状态改为 `paused`，不接收定位点。
4. 继续时会话状态改为 `active`。
5. 结束时会话状态改为 `finished`，轨迹片段写入结束时间并更新摘要。
